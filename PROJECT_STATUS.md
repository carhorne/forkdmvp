...
Status: Slice 0 implemented; F3/F4 met, F1/F2 awaiting manual hosted checks.
...
|---|---|---|
| 0 Foundation | Manual checks pending | F3, F4 met. F1 local + Vercel production met; preview pending. F2 automated redirect checks met; real email round trip pending. |
...
- F1: local dev server and https://forkdmvp.vercel.app render "10 locations" from development Supabase.
...
- Preview APP_URL is still the old (likely empty) value; choose a stable preview origin and set it, then run the F1 preview check.
