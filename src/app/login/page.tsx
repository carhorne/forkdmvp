...
export default async function Login({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <section className="login"><p className="eyebrow">WELCOME TO THE TABLE</p><h1>A little more<br />local knowledge.</h1>
    ...
    {params.error && <p className="status" role="alert">{params.error === "signout" ? "We couldn’t sign you out. Please return to the catalog and try again." : "That sign-in link has expired or was already used. Request a new code below. You can type the code instead of tapping the link."}</p>}
